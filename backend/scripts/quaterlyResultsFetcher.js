...
const getLastQuatersRevenueGrowthMetrics = async (symbol, numberOfQuaters) => {
    let results = await QuarterlyResult.find({symbol: symbol.toUpperCase(), consolidated: true}).sort({to_date: -1}).limit(numberOfQuaters * 2).lean();
    if (results.length ==  0) {
        results = await QuarterlyResult.find({symbol: symbol.toUpperCase(), consolidated: false}).sort({to_date: -1}).limit(numberOfQuaters * 2).lean();
    }
