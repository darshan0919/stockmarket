...
const getStockScripCode = async (symbol) => {
    const response = await axios.get(`${BSE_API_URL}/PeerSmartSearch/w?Type=SS&text=${symbol.trim()}`, {
        headers: {
...
const getCompanyInfo = async (scripCode) => {
    const response = await axios.get(`${BSE_API_URL}/ComHeadernew/w?quotetype=EQ&scripcode=${scripCode}`, {
        headers: {
            'Referer': 'https://www.bseindia.com/'
        }
    });
