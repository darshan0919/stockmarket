...
describe('driveDataStore sync', () => {
  test('pushes structured Drive documents, writes DTO metadata, and restores local layout', async () => {
    ...
    const push = await store.syncToDrive({ dataRoot, driveRoot });
    ...
    const pull = await store.syncFromDrive({ dataRoot, driveRoot });
  ...
  test('does not overwrite a newer Drive document with an older local copy', async () => {
  ...
  test('honors COWORK_DRIVE_SYNC=0', async () => {
