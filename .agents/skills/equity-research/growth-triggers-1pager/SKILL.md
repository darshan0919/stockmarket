...
description: Produces a single-page institutional-quality growth triggers PDF for Indian listed companies. Use for "growth triggers", "1-pager", "catalyst note", "re-rating triggers", "conviction note", "why will this stock re-rate", or when user provides a Screener.in / Stockscans link and asks for a concise analysis with triggers, timelines, and conviction tags. Full pipeline: research → 5-section analysis → 1-page PDF.
---
...
- **Uploaded PDFs:** `pdftotext -f 1 -l <PAGES> <file>.pdf out.txt`; then grep for `capacity|capex|revenue|EBITDA|margin|new product|order book|guidance|promoter` and read MDA / Directors' Report / financial highlights with `sed`.
...
Script at [`scripts/generate_pdf.py`](scripts/generate_pdf.py). Shares palette/helpers with `_shared/pdf_utils.py`. **PDF must fit on 1 page.** If it spills: cut trigger body text, not triggers. Drop to 5 triggers if still tight.
