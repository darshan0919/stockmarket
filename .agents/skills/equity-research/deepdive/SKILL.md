...
- **Uploaded PDFs:** `pdftotext -f 1 -l 999 <file>.pdf out.txt`, then `grep`/`sed` for sections.
...
Script at [`scripts/generate_report.py`](scripts/generate_report.py). Uses shared palette/helpers from `_shared/pdf_utils.py`. Fallback: `pandoc report.md -o report.pdf --pdf-engine=weasyprint`.
