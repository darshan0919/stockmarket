...
async function cmdFetch() {
  const { StockscansClient } = require(path.join(REPO, 'stock-api/src/clients/StockscansClient.js'));
  ...
  if (res.stats.failures.length) console.warn('failures:', JSON.stringify(res.stats.failures.slice(0, 5)));
  return res;
}
...
function cmdBatches() {
  ...
  batches.forEach((b) => {
    fs.writeFileSync(path.join(dir, `batch_${String(b.index).padStart(2, '0')}.txt`), renderBatch(b));
  });
  console.log(JSON.stringify({ cachedFilings: recs.length, alreadyParsed: skipped, pendingParse: pending, batches: batches.length, dir }, null, 2));
}
...
function cmdIngest() {
  ...
  for (const b of batches) {
    ...
    if (r.stats.rejected) problems.push(`batch ${b.index}: ${JSON.stringify(r.rejected.slice(0, 2))}`);
  }
...
function cmdDeploy() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ cleanUrls: true, headers: [{ source: '/(.*)', headers: [{ key: 'Cache-Control', value: 'public, max-age=0, must-revalidate' }] }] }, null, 2)
  );
  const project = arg('--project', process.env.MONTHLY_UPDATES_VERCEL_PROJECT || 'monthly-updates');
  const args = ['--yes', '--prod', '--cwd', dir, '--name', project];
  if (process.env.VERCEL_TOKEN) args.push('--token', process.env.VERCEL_TOKEN);
  try {
    const stdout = execFileSync('vercel', args, { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
...
async function main() {
  ...
  console.error(`unknown command "${cmd}" — use: fetch | batches | ingest | build | deploy | run`);
  process.exitCode = 1;
}

if (require.main === module) main().catch((e) => { console.error(e.stack || e.message); process.exit(1); });
module.exports = { cmdFetch, cmdBatches, cmdIngest, cmdBuild, cmdDeploy };
