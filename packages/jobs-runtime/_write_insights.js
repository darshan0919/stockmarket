#!/usr/bin/env node
'use strict';
// Batch-write insights + mark-processed for the 2026-07-09 watchlist-insights run.
// Workaround for lib/db.js checkpoint() EPERM-on-prune bug (sandbox forbids delete).
const fs = require('fs');
...
const arr = JSON.parse(fs.readFileSync('/tmp/fetch_result.json', 'utf8'));
const byIdx = (i) => arr[i];
...
async function run() {
  ...
  for (const [idxStr, data] of Object.entries(INSIGHTS)) {
    ...
    co.notes.push({
      ...
      tags: data.tags || [],
      category: a.category,
      announcementDescription: a.description || '',
    });
    if (!co.processedAnnouncements.includes(a.announcementId)) {
      co.processedAnnouncements.push(a.announcementId);
    }
    co.lastUpdated = new Date().toISOString();
    co.modifiedTime = co.lastUpdated;
    written += 1;
  }
  await wi.db.save(notes);
  console.log(JSON.stringify({ status: 'ok', written }));
}
