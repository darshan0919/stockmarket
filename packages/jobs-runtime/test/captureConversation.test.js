...
const JSONL = [
  JSON.stringify({
    ...
    message: {
      role: 'user',
      content: 'Analyze NSE:SWARAJENG equity thesis — margins, capex, dividend',
    },
  }),
  JSON.stringify({
    type: 'assistant',
    ...
    message: {
      role: 'assistant',
      content: [
        { type: 'thinking', thinking: 'consider order book' },
        { type: 'text', text: 'The stock looks strong on valuation and sector tailwinds.' },
      ],
    },
  }),
  JSON.stringify({
    ...
    message: { role: 'user', content: '<scheduled-task name="x">noise</scheduled-task>' },
  }),
  JSON.stringify({
    type: 'attachment',
    attachment: { file_name: 'concall.pdf', file_type: 'pdf' },
  }),
].join('\n');
...
describe('coworkTranscript.parseTranscript', () => {
  test('keeps user+assistant turns, drops scheduled-task noise, captures attachment', () => {
    ...
    expect(conv.chat_messages[1].content[0]).toEqual({
      type: 'thinking',
      thinking: 'consider order book',
    });
...
describe('captureConversation.captureOne', () => {
  const cloudConv = () => ({
    ...
    chat_messages: [
      {
        sender: 'human',
        text: 'NSE:SWARAJENG equity thesis on margin, capex, dividend, valuation',
      },
  ...
  test('non-stock conversation skipped, nothing stored', () => {
    const r = cap.captureOne(
      {
        uuid: 'x1',
        name: 'Drafting a pitch',
        chat_messages: [{ sender: 'human', text: 'help me write a friendly cover letter' }],
      },
      { source: 'cloud', now: NOW }
    );
  ...
  test('sensitive/personal (PAN + ITR) skipped even if it mentions stocks', () => {
    const conv = {
      ...
      name: 'ITR tax filing',
      chat_messages: [
        { sender: 'human', text: 'my equity capital gains, PAN ABCDE1234F, stock tax report' },
      ],
    };
  ...
  test('precomputed extract sidecar fans out notes + reports', () => {
    ...
    expect(db.find('reports', { companyId: 'NSE:SWARAJENG', type: 'chat-analysis' })).toHaveLength(
      1
    );
  });
