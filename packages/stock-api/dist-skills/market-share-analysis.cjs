...
// packages/stock-api/src/analyzers/computeConcentration.js
var require_computeConcentration = __commonJS({
  "packages/stock-api/src/analyzers/computeConcentration.js"(exports2, module2) {
    "use strict";
    function hhiClassification(hhiValue) {
      ...
      if (hhiValue < 5e3) return "Highly Concentrated";
      return "Near-Monopoly";
    }
    function _sorted(shares) {
      return shares.filter((x) => x !== null && x !== void 0).map(Number).sort((a, b) => b - a);
    }
    ...
    function hhi(namedShares, othersShare = 0, othersDistribution = "single_bucket") {
      const s = _sorted(namedShares);
      if (s.length === 0 && othersShare === 0) return 0;
      const total = s.reduce((a, b) => a + b, 0) + othersShare;
      if (Math.abs(total - 1) < 0.01 && Math.max(...s, othersShare) <= 1) {
    ...
    function computeMetrics(namedShares, othersShare = 0, othersDistribution = "single_bucket") {
      const s = _sorted(namedShares);
    ...
    function deltaBps(shareLatest, shareT5) {
      if (shareT5 === null || shareT5 === void 0) return null;
...
// packages/stock-api/src/generators/generateMarketShareHtml.js
var require_generateMarketShareHtml = __commonJS({
  "packages/stock-api/src/generators/generateMarketShareHtml.js"(exports2, module2) {
    "use strict";
    var fs = require("fs");
...
`;
    ...
    function fmtNum(value, decimals = 1, unit = "") {
      ...
      let s = decimals === 0 ? Math.round(f).toLocaleString("en-US") : f.toLocaleString("en-US", { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
      return `${s}${unit}`;
    }
    ...
    function _buildVerification(data) {
      ...
      let total = r + d + e;
      let ePct = total > 0 ? (e / total * 100).toFixed(1) : 0;
    ...
    function _buildKpiStrip(data) {
      ...
      let cells = cards.map(
        ([lbl, val, sub]) => `<div class="kpi"><div class="kpi-label">${lbl}</div><div class="kpi-value">${val}</div><div class="kpi-sub">${sub}</div></div>`
      ).join("\n");
    ...
    function _buildPlayerTable(data) {
      ...
      let thHtml = cols.map(
        (col, i) => `<th class="sortable" onclick="sortTable('player-table', ${i}, ${col[2]})">${col[0]}<span class="sort-indicator"></span></th>`
      ).join("");
    ...
    <p class="section-intro" style="margin-top:4px">
        ...
        D=derived (revenue \xF7 industry size), D-segment=carved out from
    ...
    function _buildTierTable(data) {
      ...
      let rows = tiers.map(
        (t) => `<tr><td><b>${escapeHtml(t.tier)}</b></td><td>${escapeHtml(t.share_range || "")}</td><td>${escapeHtml((t.players || []).join(", "))}</td></tr>`
      ).join("");
    ...
    function _buildDynamics(data) {
      ...
      const block = (rows, title, deltaClass) => {
        ...
        let body = rows.map(
          (r) => `<tr><td><b>${escapeHtml(r.player)}</b></td>
    ...
    function _buildThreats(data) {
      ...
      let rows = threats.map(
        (t) => `<tr><td><b>${escapeHtml(t.category)}</b></td>
    ...
    function _buildSupplyDemand(data) {
      ...
      let capexRows = capex.map(
        (c) => `<tr><td>${escapeHtml(c.player)}</td>
    ...
    function _buildSubSegments(data) {
      ...
      let rows = subs.map(
        (s) => `<tr><td><b>${escapeHtml(s.name)}</b></td>
    ...
    function _buildDataQuality(data) {
      ...
      let sourcesHtml = sources.map(
        (s) => `<li><b>${escapeHtml(s.source)}</b> (${escapeHtml(s.type || "primary")}) \u2014 
        <a href="${escapeHtml(s.url || "#")}" target="_blank" rel="noopener">link</a> \xB7 
    ...
    function createMarketShareWidget(data) {
      ...
      let dateStr = data.date || (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
...
<div class="container">
    <div class="report-header">
        ...
        <div class="report-meta">
            ${escapeHtml(geography)} \xB7 ${escapeHtml(data.definition_scope || "scope undefined")} \xB7
            Historical horizon: ${escapeHtml(data.historical_horizon || "")} \xB7
            Forward horizon: ${escapeHtml(data.forward_horizon || "")} \xB7
...
${_JS}
    ...
    module2.exports = { createMarketShareWidget };
  }
});

// packages/stock-api/bin/market-share-analysis.js
var mod_computeConcentration = require_computeConcentration();
var mod_generateMarketShareHtml = require_generateMarketShareHtml();
function main() {
