...
// packages/stock-api/src/analyzers/parseTweetDump.js
var require_parseTweetDump = __commonJS({
  "packages/stock-api/src/analyzers/parseTweetDump.js"(exports2, module2) {
    "use strict";
    var fs = require("fs");
    var path = require("path");
    var DATE_KEYS = ["created_at", "date", "datetime", "timestamp", "created", "time"];
    var TEXT_KEYS = ["text", "full_text", "tweet", "content", "body"];
    var ID_KEYS = ["id", "id_str", "tweet_id", "status_id"];
    var AUTHOR_KEYS = ["author", "user", "username", "screen_name", "handle"];
    var REPLY_ID_KEYS = ["in_reply_to_status_id", "in_reply_to_status_id_str", "in_reply_to_id", "reply_to_id"];
    var REPLY_USER_KEYS = ["in_reply_to_screen_name", "in_reply_to_user", "reply_to_user"];
    var LIKE_KEYS = ["like_count", "favorite_count", "favourite_count", "likes"];
    var RT_KEYS = ["retweet_count", "rt_count", "retweets"];
    var REPLY_COUNT_KEYS = ["reply_count", "replies"];
    function first(obj, keys, defaultVal = null) {
      if (!obj || typeof obj !== "object") return defaultVal;
      for (const k of keys) {
        if (obj[k] !== void 0 && obj[k] !== null && obj[k] !== "") {
          return obj[k];
        }
      }
      return defaultVal;
    ...
    function parseDate(s) {
      ...
      if (typeof s === "number") {
        try {
          return new Date(s < 1e12 ? s * 1e3 : s).toISOString();
        } catch (e) {
          return null;
        }
      }
    ...
    function parseApiV2(text) {
      ...
      for (const t of data) {
        if (t && typeof t.public_metrics === "object") {
          if (t.like_count === void 0) t.like_count = t.public_metrics.like_count;
          if (t.retweet_count === void 0) t.retweet_count = t.public_metrics.retweet_count;
          if (t.reply_count === void 0) t.reply_count = t.public_metrics.reply_count;
        }
...
// packages/stock-api/bin/tweet-investor-playbook.js
var mod_parseTweetDump = require_parseTweetDump();
function main() {
  const argv = process.argv.slice(2);
  if (argv.includes("--help")) {
    console.log(`Usage: tweet-investor-playbook [options]`);
    console.log(`Options:
