...
| learnyst-transcript-refresh (`packages/jobs-runtime/learnystTranscriptRefresh.js`, weekly — `jobs/Scheduled/learnyst-transcript-refresh/`)  | Learnyst bundle/module/lesson listing APIs (re-fetchable every run, cheap — ~15 calls)                                                                                                         | per-lesson AI transcript text is NOT re-derivable without another paid/rate-limited API call, so it's non-regenerable state → **new collection** `learnyst-lessons.json` (slim index) + `learnyst-lessons/<id>.json` (full transcript body — two-file pattern like `reports`, bodies run tens of KB × hundreds of lessons). Not company-scoped (personal course content, not stock research) — no `companies.json` link. Cache-first: `db.get('learnyst-lessons', id)` gates every fetch, so a lesson already stored is never re-fetched — this is the mechanism that makes the weekly job cheap on repeat runs. A lesson whose video is externally hosted on YouTube (no `content_path`) falls back to `youtubeTranscriptRefresh.js`'s yt-dlp pipeline instead — same collection, `transcriptSource: 'youtube'` on the record. See DATA_RULES §3 justification in `docs/learnyst-api-schemas.md` / this job's SKILL.md. |
| youtube-transcript-refresh (`packages/jobs-runtime/youtubeTranscriptRefresh.js`, standalone script — run manually, not yet a scheduled job) | YouTube Data API v3 `channels.list`/`playlistItems.list` (re-fetchable every run, cheap) + each video's public caption track (unofficial, no auth — see `docs/youtube-api-schemas.md`)         | per-video caption transcript text is NOT re-derivable without re-scraping the caption track, so it's non-regenerable state → **new collection** `youtube-transcripts.json` (slim index) + `youtube-transcripts/<id>.json` (full transcript body — two-file pattern like `learnyst-lessons`). Not company-scoped (channel content, not stock research) — no `companies.json` link. Cache-first: `db.get('youtube-transcripts', id)` gates every fetch, so a video already stored is never re-fetched. See DATA_RULES §3 justification in `docs/youtube-api-schemas.md`.                                                                                                                                                                                                                                                                                                                                                   |
