...
transcript for someone else's public channel (e.g. `@SOICfinance`, `@AnilLamba`, `@StockScans`) is not
...
Resolves a channel handle (e.g. `@SOICfinance`, `@AnilLamba`, or `@StockScans`) to its channel id and its
...
`https://www.youtube.com/@StockScans` or `https://www.youtube.com/@AnilLamba`) are automatically parsed and normalized.
...
skip this lookup entirely. Multiple channels can be supplied via `--channels @SOICfinance,@AnilLamba,@StockScans`
