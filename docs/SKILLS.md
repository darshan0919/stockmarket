...
- **Local First**: If the project is in-context (e.g., Cursor, local terminal), the skill will execute the local file at `packages/stock-api/bin/<skill>.js`.
...
- Storage Location: `packages/cowork-jobs/data/` locally, mirrored to Google Drive `StockMarket/cowork-jobs/v1/`.
...
- Python execution paths are deprecated and archived in `packages/stock-api/legacy/`.
- All automation must run via Node (`packages/stock-api/bin/*.js`).
- Scripts and skills read configuration purely from environment variables and command-line flags.

## 5. Adding a New Skill

1. Create a CLI entrypoint at `packages/stock-api/bin/<skill-name>.js`.
