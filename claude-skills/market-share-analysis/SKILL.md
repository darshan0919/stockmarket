...
See [`scripts/generate_market_share_html.py`](scripts/generate_market_share_html.py).

The HTML widget is the only shipped renderer. If a printed committee version is later needed, the same `data` dict can be adapted into `peer-comparison`'s ReportLab pipeline (`../_shared/pdf_utils.py` carries the same palette) — that's a one-off adaptation, not a built-in path.
