...
Inputs accepted (auto-detect via `scripts/parse_tweet_dump.py`):
...
1. Run `scripts/parse_tweet_dump.py <input-path>` to produce a normalised JSON at `/home/claude/_tweets_normalised.json`.
...
- `scripts/parse_tweet_dump.py` — format-agnostic tweet parser; outputs normalised JSON
