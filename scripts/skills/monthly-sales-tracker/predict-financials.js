...
const FINANCIAL_HISTORY = {
  'NSE:TMPV': [
    ...
    { quarter: 'Q2FY26', revenue:  72349, op:  -1404, pat:  76248, outlier: true, note: 'Exceptional PAT (demerger one-time)' },
...
function joinData(quarterlyUnits, financials) {
  ...
  for (const u of quarterlyUnits) {
    ...
    joined.push({
      ...
      note:           f.note || (u.months.length < 3 ? `${u.months.length}/3 months available (scaled ×${scaleFactor.toFixed(1)})` : ''),
    });
  }
  return joined;
}
...
async function main() {
  ...
  console.log(`   Quarters with sales data: ${allQuarters.length} (${complete.length} complete, ${partial.length} partial)`);
  ...
  if (trainSet.length < 2) {
    ...
    console.warn('  Check that the quarter labels in sales_data match FINANCIAL_HISTORY (e.g. Q2FY25, Q3FY25...)');
    console.warn('  Available quarters in sales:', allQuarters.map(q => q.quarter).join(', '));
    console.warn('  Available quarters in financials:', financials.map(f => f.quarter).join(', '));
  }

  // ── Fit quarterly models ──────────────────────────────────────────────────
  const xs = trainSet.map((r) => r.total_units);
  const modelRevenue = olsRegression(xs, trainSet.map((r) => r.revenue));
  const modelOp      = olsRegression(xs, trainSet.map((r) => r.op));
  const modelPat     = olsRegression(xs, trainSet.map((r) => r.pat));

  console.log('\n── Quarterly Regression Models ─────────────────────────────────────────');
  const fmtModel = (m, label, unit = 'Cr') => {
    const slope = m.b >= 0 ? `+${m.b.toFixed(2)}` : m.b.toFixed(2);
    console.log(`   ${label.padEnd(14)}: ${m.a.toFixed(0)} ${slope} × units  |  R² = ${(m.r2 * 100).toFixed(1)}%  (n=${m.n})`);
  };
  ...
  for (const r of records) {
    ...
    console.log(`   Units         : ${m_units.toLocaleString()} (Domestic: ${(r.domestic||0).toLocaleString()}, Exports: ${(r.exports||0).toLocaleString()}, EV: ${(r.ev||0).toLocaleString()})`);
    console.log(`   Revenue (est) : ₹${Math.round(pred_rev).toLocaleString()} Cr`);
    console.log(`   Op Profit     : ₹${Math.round(pred_op).toLocaleString()} Cr  (OPM ${opm_pct}%)`);
    console.log(`   PAT           : ₹${Math.round(pred_pat).toLocaleString()} Cr  (NPM ${npm_pct}%)`);
...
module.exports = { olsRegression, predictMonthly, toFiscalQuarter, aggregateToQuarters, joinData, FINANCIAL_HISTORY };
