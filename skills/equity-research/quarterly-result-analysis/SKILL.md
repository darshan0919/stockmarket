...
**Inventory Gains check (mandatory).** When assessing revenue/margin/profit performance for the period (Basket 1B — Margin & Profitability Triggers), explicitly check whether reported strength is inflated by inventory (stock) gains — i.e., gains from revaluing existing raw-material/finished-goods inventory upward due to rising commodity/input prices. See `references/basket_framework.md` §1B for the full checklist (Other Income jumps, margin-vs-volume mismatches, concall language, RM-cost-as-%-of-sales moves) and quantification guidance. A quarter's "blockbuster" result must be explicitly flagged as (i) inventory-gain-driven and non-recurring (`TEMPORARY` tag) or (ii) structural/operating and therefore more sustainable — this distinction belongs in the verdict chips, not buried in a sub-bullet. **Sourcing rule:** the primary evidence for this check is the "Changes in inventories of finished goods, work-in-progress and stock-in-trade" line in the standalone/consolidated P&L of the actual quarterly Result filing — fetch it via the `stock-documents-fetcher` skill (which wraps `stock-api/src/fetchers/documentsFetcher.js` / `StockscansClient.documents()`), never via web search or news-article summaries. A negative value in that line = inventory build-up = a cost tailwind (profit-inflating); a positive value = inventory drawdown = a cost headwind. Compare the YoY and QoQ swing in this line against the YoY/QoQ change in PBT — if the swing explains a large share (directionally, >30-40%) of the reported profit growth, the quarter is inventory-driven regardless of what management commentary claims. Web search/news commentary may be used only for qualitative color (e.g. management's stated reason for the swing), never as the source of the actual figures.
