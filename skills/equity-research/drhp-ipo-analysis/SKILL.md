...
19. **Subscription Status & Score** — For a closed or closing IPO (not applicable pre-close, since there's nothing to score yet): fetch the category-wise subscription figures (Anchor, QIB, sHNI, bHNI, RII, Total) via `ipoSubscriptionScanner.js`'s parsers (`parseClosedIpos`/`parseSubscriptionStatus` in `packages/jobs-runtime/ipoSubscriptionScanner.js`) against IPOPlatform's "closed IPOs" + "subscription-status" pages — **the same source `ipo-subscription-ranker` uses and the one validated against Chittorgarh's published methodology**; do not scrape a different site or hand-derive multiples yourself (see [`../ipo-subscription-ranker/references/ipo_data_sources.md`](../ipo-subscription-ranker/references/ipo_data_sources.md) for why IPOPlatform is the trusted source and what NSE's data looks like as a fallback if IPOPlatform genuinely lacks this IPO). Compute both scores via `computeDualScores()` in `packages/jobs-runtime/lib/ipoScoring.js` (`listingScore`/`listingTier` predicting listing-day gain, `cagrScore`/`cagrTier` predicting longer-run daily-CAGR performance) — never re-derive the formula inline, import and call the shared function. Then write 2-4 sentences of **insight**, not just the raw numbers: which category led (QIB-led = institutional conviction, RII-led = retail/GMP-driven hype, often the weaker signal), whether Anchor participation happened and by whom if notable, and what the STRONG/MODERATE/WEAK/POOR tier implies for near-term listing-day behavior vs whether it says anything about longer-run holding (per `ipo_ranking_framework.md`'s "Dual-score system" section — the CAGR score is intentionally noisier and less discriminating than the listing score, say so rather than overstating confidence in it).

Sections 12-19 are **not optional add-ons** — treat them as part of the core framework, extracted in the same pass as 1-11, not as follow-up work triggered only when a user asks a second time. Section 19 is naturally skipped only when the IPO genuinely hasn't closed yet (say so explicitly — "not yet applicable, IPO still open" — rather than silently omitting the section).
