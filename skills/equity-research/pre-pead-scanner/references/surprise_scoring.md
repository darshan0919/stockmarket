...
This is Steps 5 and 8: turn your independent next-quarter estimate (Step 4) into a _surprise_, and combine the surprise with the valuation and tradeability reads into a single rankable score. The discipline throughout: a surprise is a **difference from an expectation**, so it is only as good as the expectation you measure it against — and there are two expectations that matter, not one.
...
| Beat      | In line     | Street is lowballing a name that's merely doing what it guided. Positive surprise _relative to consensus_ — still tradeable. |
...
Assign each name a tier from the _composite_, not from any single leg. Use as a guide, not a rigid formula:

**TOP (high-conviction positive surprise)** — all of:

- Positive surprise versus _both_ benchmarks, or a large positive versus street with at least in-line versus guidance.
...
**BOTTOM / AVOID** — miss versus one or both benchmarks, _or_ the surprise rests on LOW-deliverability tone with no evidence, _or_ it's richly priced into a `fade` name (good result likely sold), _or_ guidance was walked back. Include for completeness; often belongs in honourable mentions rather than the ranked longs.
