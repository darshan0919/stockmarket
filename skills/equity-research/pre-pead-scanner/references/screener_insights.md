...
Screener.in publishes, for every company, an **Insights table** (key trackables insights matrix) plus a strip of **key ratios**. This is a _second, independent_ read on the same name. Used well, it sharpens a surprise call two ways: it **corroborates** a thesis you built bottom-up (raising conviction when they agree), and it **flags contradictions** worth resolving _before_ you rank a name.
...
Screener's company page is session-authenticated. The `sessionid` cookie **expires**, and an expired session silently degrades the page to a public, thinner view — so the skill must verify auth _before_ doing any work, and **halt and prompt the user** if the token is stale rather than proceeding on a degraded read.
...
| `poor-quarter-expected`                     | Screener expects a weak quarter. If _you_ project a beat, this is a **contradiction to resolve** before ranking — re-check your guidance read and one-off stripping. |
