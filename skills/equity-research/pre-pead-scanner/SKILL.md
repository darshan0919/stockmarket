...
**Output destination:** save the `{date}_pre_pead_scan.json` DTO and the rendered briefing to `<repo>/data/documents/reports/` (never the repo root or `data/`), then finish by running `yarn data:push` — it uploads to Google Drive (`StockMarket/data/v2`) and removes the local copy. See `skills/_shared/conventions.md` §6.
...
5. **Project next-quarter Revenue / OPM / PAT / EPS** — `FY guidance − YTD actual`, else run-rate × seasonality, one-offs stripped, with the **two direct PAT levers modelled explicitly**: capex-live operating leverage and balance-sheet deleverage. Method in `references/forward_estimation.md`.
   ...
   **Income Statement Signal Scan (mandatory) on the run-rate base.** Before computing `YTD actual` or the trailing run-rate used to project next-quarter OPM/PAT, run `skills/_shared/income-statement-signals.md` on the base quarter(s) against QoQ and YoY baselines — an inventory-gain-inflated, Other-Income-inflated, or tax-rate-inflated base quarter will overstate the projected surprise identically. **Sourcing rule:** pull every relevant P&L line and PBT for the base quarter(s) from the actual Result filing via `stock-documents-fetcher`, not from the concall/PPT narrative alone. If the base quarter is driven by an item that clears the shared scan's materiality bar, strip the non-recurring component before projecting and note it as a one-off flag alongside the surprise score.
