...
**Output destination:** save the `{date}_pre_pead_scan.json` DTO and the rendered briefing to `<repo>/data/documents/reports/` (never the repo root or `data/`), then finish by running `node packages/jobs-runtime/scripts/data.js push` — it uploads to Google Drive (`StockMarket/data/v2`) and removes the local copy. See `skills/_shared/conventions.md` §6.
...
5. **Project next-quarter Revenue / OPM / PAT / EPS** — `FY guidance − YTD actual`, else run-rate × seasonality, one-offs stripped, with the **two direct PAT levers modelled explicitly**: capex-live operating leverage and balance-sheet deleverage. Method in `references/forward_estimation.md`.
   ...
   **Inventory Gains check (mandatory) on the run-rate base.** Before computing `YTD actual` or the trailing run-rate used to project next-quarter OPM/PAT, test whether the base quarter(s) were inflated by inventory (stock) gains from revaluing raw-material/finished-goods inventory upward on rising commodity/input prices. An inventory-inflated base quarter will overstate the projected surprise. **Sourcing rule:** pull the "Changes in inventories" P&L line and PBT for the base quarter(s) from the actual Result filing via `stock-documents-fetcher`, not from the concall/PPT narrative alone. If the base quarter is inventory-gain-driven, strip the non-recurring component before projecting and note it as a one-off flag alongside the surprise score.
