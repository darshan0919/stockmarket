...
- Promoter-controlled entities in the _same line of business_ (Gensol pattern: Wellray Solar, Go Auto)
...
**Captive supplier exception:** When the company sells almost exclusively to one related party by business design (e.g., Swaraj Engines → M&M, MRPL → ONGC retail), high RPT-with-customer is structural and should be **YELLOW + context**, not RED. The flag should be on _changes_ in those RPT terms (price, volume share, payment terms), not the existence of the relationship.
...
- **Income Statement Signal Scan (mandatory)** — run `skills/_shared/income-statement-signals.md` (QoQ + YoY, all P&L lines and combination reads) whenever a reporting period's margin/PAT strength is being assessed. Inventory-valuation gains are one of 15 checks in that framework, alongside Other Income composition, RM-cost moves, tax-rate swings, and exceptional items — any of which can produce the same false read of operating strength. Quantify the estimated contribution to PAT/EBITDA of whatever clears the shared scan's materiality bar, and flag in the checklist whether the period's strength is driven by that non-structural effect and non-recurring (tag YELLOW even if not fraudulent — it should never be read into forward earnings power) or genuinely structural/operating.
