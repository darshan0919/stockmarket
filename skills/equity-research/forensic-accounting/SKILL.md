...
fetchDocuments('$TICKER', { types: ['Annual Report'], lastN: 3, outputDir: '$DOCS_DIR' })
...
5. **Balance sheet** — Receivables aging, inventory days, intangibles, write-offs, and the mandatory **Income Statement Signal Scan**: run `skills/_shared/income-statement-signals.md` against QoQ and YoY baselines to test whether margin/PAT strength is driven by a non-structural P&L effect (inventory revaluation gains, an Other Income spike, a tax-rate swing, an unflagged exceptional item — see `references/forensic_master_prompt.md` §5 for how this feeds the forensic verdict) rather than genuine structural/operating improvement. **Sourcing rule:** fetch the actual Result filing P&L via `stock-documents-fetcher` (`documentsFetcher.js`/`StockscansClient.documents()`) to read every literal P&L line, PBT, and PAT — do not substitute web search/news-article commentary for these figures; use search only for qualitative context. Report only what clears the shared scan's materiality bar, ranked by contribution to the PBT/PAT delta
...
mkdir -p data/forensic-accounting
bash ./skills/_shared/resolve.sh render-pdf --html report.html --pdf "data/forensic-accounting/<Company>_Output.pdf"
```
