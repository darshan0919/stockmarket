...
- **Income Statement Signal Scan (mandatory).** When cost structure or margin commentary is discussed, run `skills/_shared/income-statement-signals.md` (QoQ + YoY) rather than checking inventory gains alone — cross-check against §9's quantitative table for any line/combination (Other Income jump, RM-cost move, tax-rate swing, exceptional item) that clears the shared scan's materiality bar. Listen for management language confirming or contradicting the line-item evidence ("inventory gains," "stock gains," "favourable inventory valuation," or similar for other lines). Quantify the estimated contribution to PAT/EBITDA where derivable and flag explicitly whether the quarter's strength is (i) driven by a non-structural P&L effect and non-recurring, or (ii) structural/operating (volume, pricing power, cost efficiency, mix) — this belongs in §1's bottom line and §9's Significance column, not buried in a footnote.
