...
**Income Statement Signal Scan (mandatory).** When the diff shows margin/PAT expansion vs the prior deck, run `skills/_shared/income-statement-signals.md` on both quarters (QoQ + YoY) rather than checking inventory gains alone — the same false-beat pattern can come from an Other Income jump, a tax-rate swing, an exceptional item, or an RM-cost move, cross-checked against Dimension 3's volume data and Phase 2's concall language. Quantify the estimated PAT/EBITDA contribution of whatever clears the shared scan's materiality bar and set the "Signal" chip accordingly — tag it `Mix shift` or add an explicit non-recurring note (`Inventory-gain driven`, `Tax-rate driven`, `Non-operating beat`) rather than letting it read as a clean `Beat`. This distinction feeds directly into the Positive/Negative Surprises sections: a beat driven by a non-structural P&L effect is not a genuine positive surprise — it reverses.
