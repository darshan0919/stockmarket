...
- **User provides only a Stockscans ticker** — the skill auto-fetches the two most recent investor presentations (`stock-documents-fetcher`) and resolves the latest concall transcript via `stock-api/bin/get-concall-transcript-url.js`
...
Handle the transcript resolver's output: on success, fetch and read the
document at `documentUrl` for Phase 2. On `error`, that quarter's transcript
genuinely isn't filed yet (rare) — confirm with the user before proceeding,
then proceed to Phase 1/3 without a concall and skip Phase 2 (note the gap
rather than fabricating reconciliation).
