#!/usr/bin/env python3
"""
search_stockscans.py — Extraction pass for the ask-stockscans skill.

Zero-LLM lexical (TF-IDF-style) search over the full transcript text of StockScans'
official YouTube videos stored in `data/youtube-transcripts/*.json`. Given a
...
link), so the caller (ask-stockscans SKILL.md or ask-expert) only has to read a handful of
...
Index caching: term-frequency + document-frequency statistics for the StockScans
channel corpus are cached to `data/cache/ask-stockscans/index.json`
keyed by a corpus fingerprint (record count + latest modifiedTime across StockScans
records in `data/youtube-transcripts.json`).

Usage:
    python3 search_stockscans.py --query "concall scans and research ai" \\
        --data-root /path/to/stockmarket/data --top 8

    # Force a full reindex (e.g. right after `yarn youtube-transcript-refresh`):
    python3 search_stockscans.py --query "..." --data-root data --top 8 --reindex
"""
import argparse
import hashlib
import json
...
def filter_stockscans_index(youtube_index):
    """Filter youtube-transcripts index down to StockScans channel."""
    return {
        rec_id: rec
        for rec_id, rec in youtube_index.items()
        if (rec.get("channelHandle") or "").lower().replace("@", "") == "stockscans"
        or (rec.get("channelTitle") or "").lower() == "stockscans"
        or rec.get("channelId") == "UCrgKbrMD08iK8HI4oFA--kA"
    }


def corpus_fingerprint(stockscans_index):
    n = len(stockscans_index)
    latest = ""
    for rec in stockscans_index.values():
        mt = rec.get("modifiedTime") or rec.get("fetchedAt") or ""
        if mt > latest:
            latest = mt
    raw = f"{n}:{latest}"
...
def build_index(data_root, stockscans_index):
    """Read every StockScans transcript body once, build TF-IDF doc vectors + df table."""
    docs = {}
    df = {}
    ...
    if os.path.exists(youtube_dir):
                ...
                with open(p, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        try:
                            ...
                            rec_id = body.get("id")
                            if not rec_id or rec_id not in stockscans_index:
                                continue
                            rec = stockscans_index[rec_id]
                            ...
                            add_doc(
                                ...
                                tokens,
                                {
                                    ...
                                    "collection": rec.get("channelTitle") or body.get("channelTitle") or "StockScans",
        ...
        for rec_id, rec in stockscans_index.items():
            if rec_id in docs:
                continue
            ...
            add_doc(
                ...
                tokens,
                {
                    ...
                    "collection": rec.get("channelTitle") or "StockScans",
...
def load_or_build_index(data_root, cache_path, stockscans_index, force_reindex):
    fp = corpus_fingerprint(stockscans_index)
    ...
    index = build_index(data_root, stockscans_index)
...
def main():
    ...
    stockscans_index = filter_stockscans_index(all_youtube_index)

    if not stockscans_index:
        print(
            json.dumps(
                {
                    "corpusSize": {"totalVideos": 0, "indexedWithTranscripts": 0},
                    "results": [],
                    "message": "No StockScans videos found in data/youtube-transcripts.json yet. "
                    "Run `yarn youtube-transcript-refresh` to populate.",
                }
            )
        )
        sys.exit(0)

    cache_path = os.path.join(args.data_root, "cache", "ask-stockscans", "index.json")
    index = load_or_build_index(args.data_root, cache_path, stockscans_index, args.reindex)
