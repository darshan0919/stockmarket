...
6.  **Data Lifecycle:** All generated data lives under `<repo>/data/` (never repo root, cwd, or ad-hoc folders). Store ONLY metadata, links, LLM/analyst outputs, and non-regenerable state. Anything re-derivable at runtime via script/API (raw API dumps, downloaded PDFs, market data) is NOT stored — raw run artifacts go to `data/runs/`, heavy frequently-read derivables to `data/cache/`. **Target the 100 KB – 10 MB Sweet Spot Rule:** Never write loose per-item files (< 100 KB) into synced folders. Partition stores as single JSONL (< 10 MB), 16 hex shards (> 10 MB, e.g. `learnyst-lessons`, `youtube-transcripts`, `pdf-text`, `monthly-updates-text`), quarterly (`reports`), or annually (`events`, `conversations`, `runs`). Every sync audits stores against the 10 MB ceiling and flags necessary migrations. Before a skill finishes it MUST run `yarn data:push` — idempotent, PUSH-ONLY sync to Drive `StockMarket/data/v2` with adaptive parallel batching (max 16 workers, throttled to 25 MB in-flight payload). Everything under `data/` is kept locally (full 1:1 mirror); nothing is ever deleted in a write path (Cowork mounts throw EPERM on delete — an inline `rm` can abort the save that triggered it). Re-fetchable source documents (downloaded PDFs) are simply not persisted under `data/` in the first place — read them from a temp dir outside the mirror.
...
24. **No script in this repo may call an LLM provider API directly — ever. Every
    ...
    reads these records directly out of the sharded`data/events-YYYY-MM.json` files the same way any other events consumer would, aggregates the last 7
    ...
    SKILL.md's final step calls`track_invocation.py`(or`recordTokenUsage.js` directly) — treat a job with real runs in the window but zero
...
25. **Four more job-level metrics, wired the same way as §23/§24 — all flushed
    ...
    and resets that job's in-memory bucket. - **Run duration.** `tokenUsageTracker.flush(jobName, {date, note,
