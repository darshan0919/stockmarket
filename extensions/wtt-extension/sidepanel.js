...
function extractJSON(raw) {
  ...
  throw new SyntaxError(
    `Failed to extract JSON from response (length=${raw.length}): ${raw.slice(0, 120)}...`
  );
}
...
async function callClaude(system, userContent, opts = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401)
        throw new ClaudeAPIError(`Authentication failed — check your API key`, res.status, true);
      if (res.status === 429)
        throw new ClaudeAPIError(`Rate limited — wait a moment and retry`, res.status, false);
      if (res.status >= 500)
        throw new ClaudeAPIError(`Claude server error (${res.status})`, res.status, false);
      const isBilling =
        body.includes('credit balance') ||
        body.includes('billing') ||
        body.includes('purchase credits');
      if (isBilling)
    ...
    if (e.name === 'AbortError')
      throw new ClaudeAPIError(`Claude request timed out after ${timeoutMs / 1000}s`, 0, false);
    throw e;
  } finally {
    clearTimeout(timer);
  }
...
async function startPipeline(symbol) {
  ...
  try {
    ...
    if (sortedDates.length < 2)
      throw new Error('Need at least 2 quarters of data on stockscans.in');

    const quarters = sortedDates.map((d) => ({
      date: d,
      label: dateToQuarter(d),
      docs: byDate[d],
    }));
...
Return ONLY valid JSON — no markdown fences, no preamble.`;
    ...
    for (let i = 0; i < quarterData.length; i++) {
      ...
      setStep(
        'extract',
        'active',
        `${i + 1} / ${quarterData.length} · ${q.label}${hasNotes ? ' + notes' : ''}`
      );
...
Analyse ALL attached documents and any supplementary notes above for this quarter and return JSON:
{
  ...
  "financials": {
    ...
    "attrition_pct": null
      ...
      try {
        const parsed = await callClaudeWithDocsJSON(EXTRACT_SYS, q.pdfs, prompt, {
          maxTokens: 4096,
        });
    ...
    setStep(
      'extract',
      'done',
      `${successCount}/${quarterData.length} quarters analysed${extractFails ? ` (${extractFails} failed)` : ''}`
    );
    setProgress(60);
...
- wtt_score = execution_score × 0.5 + language_score × 0.3 + consistency_score × 0.2`;
    ...
    setStep(
      'score',
      'done',
      `${quarterScores.length} quarters scored${scoreFails ? ` (${scoreFails} failed)` : ''}`
    );
    setProgress(85);
...
Return JSON:
{
  ...
  "overall_verdict": "2–3 sentences. Direct, investment-grade, specific."
    ...
    let overall = {
      ...
      top_red_flags: [],
      strengths: [],
      overall_verdict: 'Analysis complete.',
    };
    try {
...
<body>
  ...
  ${
    quarterScores.length
      ? (() => {
          const avg = (k) =>
            Math.round(quarterScores.reduce((s, q) => s + (q[k] || 0), 0) / quarterScores.length);
          const e = avg('execution_score'),
            l = avg('language_score'),
            c2 = avg('consistency_score');
...
function render() {
  ...
  if (S.symbol)
    tbr.append(mk('span', 'mono muted', { style: 'font-size:11px', textContent: S.symbol }));
  tb.append(tbr);
  panel.append(tb);
...
function renderSetup(body) {
  ...
  const btn = mk('button', 'btn', {
    textContent: 'Save & Start Analysis →',
...
function renderRunning(body) {
  ...
  S.steps.forEach((s) => {
    ...
    const labelColor =
      s.status === 'active' ? 'var(--text)' : s.status === 'done' ? 'var(--textM)' : 'var(--textD)';
    label.append(mk('span', '', { textContent: s.label, style: `color:${labelColor}` }));
    if (s.detail)
      label.append(
        mk('span', 'muted', { style: 'font-size:11px;margin-left:8px', textContent: s.detail })
      );
...
function renderDone(body) {
  ...
  right.append(
    mk('div', '', {
      style:
        'font-size:10px;color:var(--textM);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px',
      textContent: 'Walk the Talk Score',
    }),
    mk('div', '', {
      style: `font-family:var(--ff-d);font-size:20px;font-weight:800;color:${col}`,
      textContent: overall.credibility_rating + ' Credibility',
    }),
    mk('div', 'muted', {
  ...
  if (quarterScores.length) {
    ...
    [
      ['Execution', 'execution_score', '50%'],
      ['Language', 'language_score', '30%'],
      ...
      row.append(
        left,
        mk('span', 'mono', { style: `font-size:16px;font-weight:500;color:${c}`, textContent: v })
      );
  ...
  if (overall.top_red_flags?.length) {
    ...
    c.append(
      mk('div', '', {
        style:
          'font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:var(--red);margin-bottom:8px',
        textContent: 'Red Flags',
      })
    );
    overall.top_red_flags.forEach((f) =>
      c.append(mk('div', 'item item-r', { style: 'margin-bottom:4px', textContent: f }))
    );
    gr.append(c);
    gridHasContent = true;
  }
  if (overall.strengths?.length) {
    ...
    c.append(
      mk('div', '', {
        style:
          'font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:var(--green);margin-bottom:8px',
        textContent: 'Strengths',
      })
    );
    overall.strengths.forEach((s) =>
      c.append(mk('div', 'item item-g', { style: 'margin-bottom:4px', textContent: s }))
    );
    gr.append(c);
    gridHasContent = true;
  }
  ...
  // Action buttons
  const dlBtn = mk('button', 'btn-ghost', {
    textContent: '\u2193 Re-download Report',
    style: 'margin-top:4px',
  });
  dlBtn.addEventListener('click', () => downloadReport(S.results));
  body.append(dlBtn);

  const newBtn = mk('button', 'btn-ghost', {
    textContent: '\u2190 New analysis',
    style: 'margin-top:4px',
  });
...
function makeQRow(q) {
  ...
  right.append(
    mk('span', '', {
      style: 'font-size:10px;color:var(--textD)',
      textContent: `E:${q.execution_score} L:${q.language_score} C:${q.consistency_score}`,
    }),
    mk('span', '', {
      style: 'color:var(--textD);font-size:11px',
      textContent: isOpen ? '\u25B2' : '\u25BC',
    })
  );
  head.append(left, right);
  head.addEventListener('click', () => {
    S.expandedQ = isOpen ? null : q.quarter;
    render();
  });
  wrap.append(head);
  ...
  if (isOpen) {
    ...
    const sec = (label, cls, arr) => {
      ...
      const colorVar =
        cls === 'item-g' ? 'var(--green)' : cls === 'item-r' ? 'var(--red)' : 'var(--amber)';
      detailBody.append(
        mk('div', '', {
      ...
      arr.forEach((t) =>
        detailBody.append(mk('div', `item ${cls}`, { style: 'margin-bottom:3px', textContent: t }))
      );
    };
...
function makeSVGGauge(score) {
  ...
  const r = 34,
    circ = 2 * Math.PI * r,
    pct = Math.min(1, Math.max(0, (score || 0) / 100));
  const col = scoreColor(score);

  const bg = document.createElementNS(NS, 'circle');
  bg.setAttribute('cx', '45');
  bg.setAttribute('cy', '45');
  bg.setAttribute('r', r);
  bg.setAttribute('fill', 'none');
  bg.setAttribute('stroke', 'var(--bg3)');
  bg.setAttribute('stroke-width', '7');
  bg.setAttribute('stroke-dasharray', `${circ * 0.75} ${circ * 0.25}`);
  bg.setAttribute('stroke-dashoffset', `${circ * 0.875}`);
  bg.setAttribute('transform', 'rotate(135 45 45)');

  const fg = document.createElementNS(NS, 'circle');
  fg.setAttribute('cx', '45');
  fg.setAttribute('cy', '45');
  fg.setAttribute('r', r);
  fg.setAttribute('fill', 'none');
  fg.setAttribute('stroke', col);
  fg.setAttribute('stroke-width', '7');
  fg.setAttribute('stroke-linecap', 'round');
  fg.setAttribute('stroke-dasharray', `${circ * 0.75} ${circ * 0.25}`);
  fg.setAttribute('stroke-dashoffset', `${circ * 0.875 + circ * 0.75 * (1 - pct)}`);
  fg.setAttribute('transform', 'rotate(135 45 45)');
  fg.style.transition = 'stroke-dashoffset 1.2s ease';

  const num = document.createElementNS(NS, 'text');
  num.setAttribute('x', '45');
  num.setAttribute('y', '40');
  num.setAttribute('text-anchor', 'middle');
  num.setAttribute('font-family', 'Syne,sans-serif');
