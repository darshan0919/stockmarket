...
const ORDER_PAT =
  /award_of_order|receipt_of_order|award of order|receipt of order|bagging of order|bags?\s+(an?\s+)?order|work order|purchase order|letter of intent|letter of award|loa\b|\bloi\b|secures?\s+(an?\s+)?(order|contract)|wins?\s+(an?\s+)?(order|contract)|order book|contract from|order from|repeat order|export order/i;
const RESULT_PAT =
  /financial results?|board meeting outcome.*result|results? for the (quarter|year)|unaudited|audited results/i;
const CAPRAISE_PAT =
  /preferential issue|preferential allotment|qualified institutions? placement|\bqip\b|warrants?|fund rais|rights issue/i;
const SAST_PAT =
  /sast|substantial acquisition|reg(ulation)?\.?\s*29|reg(ulation)?\.?\s*31\b|takeover/i;
const PIT_PAT = /insider trading|\bpit\b|reg(ulation)?\.?\s*7\s*\(2\)/i;
const RATING_PAT =
  /credit rating|rating action|rating update|care ratings|icra|crisil|india ratings|brickwork|acuite/i;
const DEBT_PAT =
  /prepayment|pre-payment|debt[- ]free|repayment of (term loan|debt|ncd)|redemption of (ncd|debenture)|reduc\w+ (of )?debt|deleverag/i;
const CAPACITY_PAT =
  /commercial production|commissioning|commenc\w+ (of )?(commercial )?production|capacity expansion|new (manufacturing )?(plant|facility|unit)|greenfield|brownfield|capex|expansion of capacity|land (acquisition|allotment|purchase).{0,40}(plant|facility|project|expansion)|foundation stone|groundbreaking/i;
const APPROVAL_PAT =
  /usfda|us fda|form 483|\beir\b|establishment inspection|eu gmp|who[- ]gmp|anda approval|\bcep\b|dcgi|nmpa|anvisa|pmda|tga approval|marketing authori[sz]ation|patent (grant|granted)|pli scheme|production linked|bis certification|peso|dgca|cdsco/i;
const MNA_PAT =
  /acquisition|amalgamation|merger|demerger|scheme of arrangement|slump sale|stake (purchase|acquisition)|open offer|delisting|joint venture|\bjv\b|strategic (partnership|alliance|investment)|technology (transfer|licens|tie[- ]up)|\bmou\b|memorandum of understanding|diversification/i;
const BUYBACK_PAT = /buy[- ]?back/i;
const VOLMOVE_PAT =
  /clarification regarding (price|volume) movement|unusual (price|volume)|spurt in (price|volume)/i;

const RISK_PAT =
  /pledge|encumbrance|resignation of (statutory )?auditor|auditor.{0,30}resign|resignation of (the )?(cfo|chief financial|managing director|company secretary)|search (and|&) seizure|income tax (raid|search)|gst.{0,20}(raid|search|notice)|\bed\b raid|enforcement directorate|sebi (order|show cause)|default|insolvency|nclt|winding up|fire (at|in|broke)|accident at|suspension of (trading|operations)|credit rating.{0,40}downgrad/i;
const NOISE_PAT =
...
const ANALYST_MEET_PAT =
  /analyst|investor meet|institutional investor|plant visit|earnings call|conference call|investor presentation/i;

const _VALUE_RE =
  /(?:rs\.?|inr|₹)\s*([\d,]+(?:\.\d+)?)\s*(crore|cr\b|lakh|lacs?|million|mn\b|billion|bn\b)?|usd\s*([\d,]+(?:\.\d+)?)\s*(million|mn\b|billion|bn\b)?|\$\s*([\d,]+(?:\.\d+)?)\s*(million|mn\b|billion|bn\b)?/gi;
const USD_INR = 86.0;
...
function classify(ann, company) {
  ...
  const marquee = [
    ...new Set([...findNames(tl, MARQUEE_GLOBAL), ...findNames(tl, MARQUEE_INDIA)]),
  ].sort();
  ...
  if (RISK_PAT.test(tl) && !NOISE_PAT.test(tl)) {
    ...
    if (/downgrad/.test(tl)) {
      return _alert(
        ...
        'RISK',
        'RISK',
        'Credit rating downgrade — structural negative.',
        value_cr,
        marquee,
        themes
      );
  ...
  if (SAST_PAT.test(tl) || PIT_PAT.test(tl)) {
    ...
    if (m) ent = m[1].trim();
    const instKw =
      /mutual fund|\bmf\b|asset management|\bamc\b|capital|investments?|insurance|pension|\bfpi\b|\bfii\b|fund\b|advisors|partners|ventures|\bllp\b/i;
  ...
  if (CAPACITY_PAT.test(tl)) {
    const sev =
      (value_cr && ttm_rev && value_cr / ttm_rev > 0.15) || themes.length ? 'HIGH' : 'MEDIUM';
    return _alert(
      ann,
      company,
      'CAPACITY EXPANSION',
  ...
  if (MNA_PAT.test(tl)) {
    ...
    const sev =
      marquee.length || (value_cr && ttm_rev && value_cr / ttm_rev > 0.15) ? 'HIGH' : 'MEDIUM';
...
function priceVolumeAlerts(companies) {
  ...
  for (const c of companies) {
    ...
    if (r1d !== null && r1d !== undefined && r1d >= THRESHOLDS.ret_1d_high) {
      out.push(
        _alert(
          { date: '', title: `1D move ${r1d > 0 ? '+' : ''}${r1d.toFixed(1)}%` },
          c,
          'PRICE/VOLUME',
          'MEDIUM',
      ...
      out.push(
        _alert(
          { date: '', title: `1W move ${r1w > 0 ? '+' : ''}${r1w.toFixed(1)}%` },
          c,
          'PRICE/VOLUME',
          'MEDIUM',
