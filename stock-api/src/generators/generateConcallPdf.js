...
function flagColor(rating) {
  ...
  if (r.includes('RED') || r === 'MISSED' || r === 'DODGED' || r === 'NO')
    return INSTITUTIONAL_LIGHT.bad;
  if (r.includes('YELLOW') || r === 'MISSING' || r === 'TOO EARLY' || r === 'PARTIAL')
...
function _themeTrackerTable(themes, quarters) {
  ...
  themes.forEach((t) => {
    const row = [t.theme || ''];
    quarters.forEach((q) =>
      row.push((t.stances && t.stances[q] ? String(t.stances[q]) : '').substring(0, 30))
    );
