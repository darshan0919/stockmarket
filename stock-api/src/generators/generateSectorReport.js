...
async function createSectorReport(sectorName, subTheme, reportMarkdown, outputPath, opts = {}) {
  ...
  for (let line of lines) {
    if (
      line.trim().startsWith('[Analyst View]') ||
      line.trim().toLowerCase().startsWith('**[analyst view]**')
    ) {
      const txt = line.trim().replace(/^\\*\\*\\[analyst view\\]\\*\\*/i, '[Analyst View]');
      customizedMd.push(
        `<div style="color: ${INSTITUTIONAL_DARK.secondary}; font-weight: bold; margin-bottom: 2mm; margin-left: 4mm;">${formatInlineMarkdown(txt)}</div>`
      );
    } else {
