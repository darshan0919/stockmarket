...
function flagColor(rating) {
  ...
  if (r.includes('YELLOW') || r === 'PARTIAL' || r === 'PARTIAL MATCH')
    return INSTITUTIONAL_LIGHT.warn;
  if (r.includes('GREEN') || r === 'NO MATCH' || r === 'NO' || r === 'N/A')
    return INSTITUTIONAL_LIGHT.good;
...
function _checklistTable(items) {
  ...
  headers.forEach(
    (h) =>
      (html += `<th style="border: 1px solid ${INSTITUTIONAL_LIGHT.border}; padding: 4px;">${h}</th>`)
  );
  html += `</tr>`;
...
function _piotroskiTable(piotroski) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const bg = isHeader
      ? INSTITUTIONAL_LIGHT.primary
      : rowIndex % 2 === 0
        ? INSTITUTIONAL_LIGHT.tint
        : '#ffffff';
