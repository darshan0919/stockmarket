...
const latestResults = results.filter((r) => r.xbrl && r.consolidated === 'Consolidated');
```
...
```jsx
{
  quarter.fiscal_year !== prevYear && (
    <th className="border-l-4 border-blue-500">FY {quarter.fiscal_year}</th>
  );
}
```
...
```jsx
<button onClick={() => scrollToYear(2023)}>Jump to 2023</button>
```
