...
Order book tracking and analysis is available in the dedicated **Orders** tab on stock detail pages. The feature tracks pending order books by monitoring order inflows, completions, and cancellations from the latest reported order book. The Orders tab uses `OrdersTab.js` with sub-components (`OrderBookView`, `OrderAnnouncements`, `OrderDetails`, `QuarterView`, `OrderDownloads`) in `frontend/components/stock/orders/`.
...
3. User selects "Order Book" view mode
4. OrderBookView fetches/uses data from ordersAPI
5. Displays loading spinner during fetch
6. Renders order inflow cards + order list
7. Shows info note about data sources
```
